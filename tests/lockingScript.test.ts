...
describe("lockingScript", () => {
  ...
  it("should build a valid locking script", () => {
    ...
    expect(lockingScript.equals(expectedScript)).toBe(true);
  });
  ...
  it("should throw an error for invalid input types", () => {
    // @ts-ignore
    expect(() => buildLockingScript("invalidAddress", Buffer.from(delegatorKey, "hex"), Buffer.from(validatorKey, "hex"), lockBlockNumber, validatorNodeIndex, nonce))
    .toThrow("Invalid input types");

    // @ts-ignore
    expect(() => buildLockingScript(Buffer.from(ownerEvmAddress.slice(2), "hex"), "invalidDelegatorKey", Buffer.from(validatorKey, "hex"), lockBlockNumber, validatorNodeIndex, nonce))
    .toThrow("Invalid input types");

    // @ts-ignore
    expect(() => buildLockingScript(Buffer.from(ownerEvmAddress.slice(2), "hex"), Buffer.from(delegatorKey, "hex"), "invalidValidatorKey", lockBlockNumber, validatorNodeIndex, nonce))
    .toThrow("Invalid input types");
  });
